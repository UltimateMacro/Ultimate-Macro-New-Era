...
SLE_ValidateStrategyText(text) {
    ...
    if (Trim(text) = "")
        throw Error("Strategy text is empty.")
    if (StrLen(text) * 2 > SLE_StrategyMaxBytes)
        throw Error("Strategy text is larger than the 5 MB editor safety limit.")
    if SLE_StrategyHasNul(text)
        throw Error("Strategy contains embedded NUL bytes inside its text and cannot be edited safely.")
    ...
    return { Placements: spawnCount, CalibrationOnly: calibrationOnly }
}

SLE_LoadValidatedStrategy(path) {
    global SLE_StrategyMaxBytes
    ...
    checked := SLE_ValidateStrategyText(loaded.Text)
    ...
    return {
        ...
        CalibrationOnly: checked.CalibrationOnly
    }
}