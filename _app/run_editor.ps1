...
function Find-AutoHotkeyV2 {
    ...
    # Ultimate Macro v1.3.x releases bundle a portable AHK v2 executable. Use it as a
    # fallback so Strategy Lab does not force a redundant system-wide install.
    try {
...
function Ensure-WebViewToo {
    ...
    # Exact Git blob ids from the immutable commit above. A size check alone can accept
    # corrupted/substituted files, including the native WebView2 loader DLLs.
    $files = @(
...
function Invoke-StrategyLabSelfTest([string]$AhkPath) {
    ...
    if ($proc.ExitCode -ne 0) {
        # Re-run visibly once so the user gets the exact parser/contract failure.
        Write-Host ''
