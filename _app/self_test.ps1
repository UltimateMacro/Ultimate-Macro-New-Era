...
if ($js -notmatch 'function projectionAt\(p\)' -or $js -notmatch 'const projection = projectionAt\(p\)' -or $js -notmatch 'const size = footprintSize\(p\)') { Fail 'Web renderer position-aware footprint projection is missing' } else { Pass 'Web renderer position-aware footprint projection' }
