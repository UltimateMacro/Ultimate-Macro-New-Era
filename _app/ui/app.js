/* ============================================================================
   Ultimate Macro — Strategy Lab (WebView front end)

   Hosted by StrategyEditorHost.ahk. Every `ahk.*` call below is a callback
   registered there with AddCallbackToScript; nothing here touches the game.
   ========================================================================= */
(() => {
  'use strict';
  ...
  // Marker fills. Slot 1 keeps the macro accent so the primary tower reads first.
  ...
  /* Windows paths are long and bidirectional CSS truncation mangles the
     separators, so shorten them here and keep the full value in the tooltip. */
  const SEPARATORS = /[\\/]+/;
  ...
  /* Messages pushed by the AHK host. It must never open a native dialog from a
     bridge callback (that wedges WebView2's host-object queue), so it reports
     here instead. */
  ...
  /* Marks a button busy and returns the exact undo for that call, so a finished
     async action can never leave a control stuck disabled. */
  ...
  /* WebView2's own `app-region: drag` does nothing in this host, so the borderless
     window is dragged the way Main.ahk drags its own. Only the grab is reported:
     AHK follows the cursor from there, because one bridge call per pointermove
     was slow enough to make the window visibly lag behind the mouse. */
  els.titlebar.addEventListener('pointerdown', (ev) => {
    // No preventDefault: it would also suppress the dblclick that toggles maximize.
  ...
  function buildMapMenu(preferred = '') {
    ...
    // A strategy may name a map that is not in the catalog. Keep it selectable.
  ...
  function projectionAt(p) {
    ...
    // Coefficients use normalized CLIENT position. canonicalPoint() already maps any
    // recorded client size into the verified 1920×1009 contract, so calibration survives
    // a strategy recorded at another supported client size without re-fitting.
  ...
  function pairCollides(a, b) {
    ...
    // Approximate the local inverse ground metric from the learned screen Jacobian.
    // With global X=Y PPU this reduces exactly to the legacy circular collision test.
  ...
  // `element`/`placement` are passed while building, before state.footprintEls exists.
  function updateFootprintPosition(index, element, placement) {
    ...
    // Perspective is position-dependent: a drag can change the visible ring diameter.
  ...
  async function pollReplayStatus(epoch) {
    ...
    } catch { /* the runner writes its status file asynchronously; keep polling */ }
  ...
  async function captureCalibration(stage, options = {}) {
    ...
    try {
      ...
      if (stage === 'baseline') {
        ...
        cal.manualPath = '';
        cal.marks = [];
        // The captured client rectangle IS the coordinate space the macro records in.
  ...
  /* Reads the cyan placement ring radius straight out of the captured screenshot
     so the editor's ring size matches what TDS actually drew. */
  ...
  function addCalibrationMark(clientX, clientY) {
    ...
    // Ultimate Macro records MouseGetPos in CLIENT mode and the screenshot is the exact
    // Roblox client rectangle, so image coordinates already are the .strat X/Y.
    const pt = screenToWorld(clientX, clientY);
  ...
  window.addEventListener('keydown', (ev) => {
    ...
    // Editing shortcuts must not fight the caret while a field has focus.
  ...
  // The AHK host drives these two entry points.
  ...
  async function loadBrandIcon() {
    ...
    } catch { /* the lettered tile stays as the fallback */ }
  }
