...
; Ultimate Macro v1.3.x records and replays tower coordinates in Roblox CLIENT space.
; Keep the calibration runner on the same contract so client-window borders/origin never
; leak into .strat coordinates.
...
SLE_GetMacroContract() {
    ...
    ; Accept both historical cached Slots and the current placement-time dynamic
    ; client geometry helper. The latter fixes stale Hotbar OFF coordinates when
    ; Roblox is resized/moved after startup.
...
SLE_UpgradePlacedTower(root, hwnd, x, y, upgradeKey) {
    ...
    ; Cash, animation timing, and UI focus can all delay the upgrade button. Reopen
    ; the same tower and retry the direct button click instead of relying on a keybind.
    Loop 5 {
        Click(x, y)
        Sleep(180)
