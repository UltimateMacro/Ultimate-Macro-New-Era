...
RuntimeLogExportBundle(destination := "") {
    ...
    if (destination = "")
        destination := A_Temp "\\UltimateMacro-logs-" FormatTime(, "yyyyMMdd-HHmmss") "-pid" DllCall(
            "Kernel32\\GetCurrentProcessId", "UInt") ".txt"
    ...
    try {
        ...
        files := [{ label: "Current session", path: RuntimeLogState.SessionFile }, { label: "Persistent runtime log",
            path: RuntimeLogState.StoreFile }, { label: "Last crash report", path: RuntimeLogState.Dir "\\last-crash.log" }
        ]
        for item in files {
