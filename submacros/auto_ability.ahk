...
UseAbilities() {
    ...
    v := aGui.Submit(false)
    chainInterval := AbilityInterval(v.chainInterval, 10)
    beatInterval := AbilityInterval(v.beatInterval, 26)

    if (v.useChain && A_TickCount - LastChainTime > chainInterval * 1000) {
        if (waitForTowerUI()) {
    ...
    if (v.useBeat && A_TickCount - LastBeatTime > beatInterval * 1000) {
        if (waitForTowerUI()) {
            SendEvent("{RButton Up}")
            Click(ScaleX(unfocusX), ScaleY(unfocusY))
            Sleep(300)
...
AbilityInterval(value, fallback) {
    text := Trim(String(value))
    if (text = "" || !IsNumber(text))
        return fallback
    return Max(1, Integer(text))
}
