...
global OfficialRemoteBaseUrl := "https://remote.ultmacro.xyz"
...
if (mode = "ask-file") {
    ...
    try {
        ...
        if (baseUrl != OfficialRemoteBaseUrl || token = "")
...
SetupOfficialRemote(connectionCode := "", interactive := true) {
    ...
    try {
        ...
        if (baseUrl != OfficialRemoteBaseUrl)
            throw Error("That connection code is not for the official Ultimate Macro Remote service.")
...
RunWorker() {
    global RemoteSettings, RemoteDir, ClientVersion, OfficialRemoteBaseUrl
    baseUrl := RTrim(IniRead(RemoteSettings, "Remote", "BaseUrl", ""), "/")
    token := LoadProtectedToken()
    installId := GetOrCreateInstallId()
    if (baseUrl != OfficialRemoteBaseUrl || token = "")
